...
async function seed() {
    try {
        ...
        const backupData = {
            users: await models_1.User.find({}),
            categories: await models_1.Category.find({}),
            products: await models_1.Product.find({}),
            customers: await models_1.Customer.find({}),
            settings: await models_1.Setting.find({})
        };
        const fs = require('fs');
        ...
        const washingMachines = await models_1.Category.create({ name: 'Кир ювиш машиналари', parentId: appliances._id });
        const tvs = await models_1.Category.create({ name: 'Телевизорлар', parentId: electronics._id });
        const airConditioners = await models_1.Category.create({ name: 'Кондиционерлар', parentId: appliances._id });
        const microwaves = await models_1.Category.create({ name: 'Микроволновкалар', parentId: appliances._id });
        const refrigerators = await models_1.Category.create({ name: 'Музлатгичлар', parentId: appliances._id });
        const smallAppliances = await models_1.Category.create({ name: 'Кичик маиший техника', parentId: appliances._id });
